...
In the general case, Kruskal’s algorithm has three main steps:

Initializing the disjoint sets – This step takes $O(|V|)$, which is minimal in the overall runtime.

Sorting the edges – Sorting typically requires $O(|E| \log |E|)$ time, making it the dominant factor in the algorithm’s complexity.

Processing each edge to decide if it joins two disjoint components. This involves Find-Set and Union operations from the disjoint set data structure, which have a combined time complexity of $O((|V| + |E|) \alpha(|V|))$, where $\alpha$ is a very slowly growing function, as explained in Section 21.4.

So the total time complexity can be expressed as: $$ O(|E| \log |E|) + O((|V| + |E|) \alpha(|V|)) $$ Since $O(|E| \log |E|)$ dominates $O((|V| + |E|) \alpha(|V|))$, we can simplify the overall complexity to $O(|E| \log |E|)$.

However, if all edge weights are integers in the range $1$ to $|V|$, we can use counting sort (see Section 8.2) to sort the edges in $O(|E|)$ time. This reduces the sorting time, so it no longer dominates the runtime. In this case, the disjoint set operations determine the overall complexity, leading to $O(|E| \alpha(|V|))$.

If the edge weights are instead bounded by a constant $W$, the same reasoning applies. Counting sort will still allow $O(|E|)$ sorting, leaving the disjoint set operations as the primary time cost, so the complexity becomes $O(|E| \alpha(|V|))$.

Summary:

* With arbitrary edge weights, the runtime is $O(|E| \log |E|)$.
* With edge weights in the range $1$ to $|V|$, counting sort reduces the sorting time to $O(|E|)$, making the runtime $O(|E| \alpha(|V|))$, dominated by the disjoint set operations.
* With edge weights in the range $1$ to $W$, it the same as above.
